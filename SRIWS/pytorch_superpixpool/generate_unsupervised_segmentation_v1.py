...
def run(input_image_path, resdir='unseg'):
    ...
    respath = os.path.join(r'.\datasets_gf\seg_Mynet', ibase)
...
def run_tif(input_image_path, resdir='unseg'):
    start_time0 = time.time()
    ...
    tensor = image.transpose((2, 0, 1))
    tensor = tensor.astype(np.float32) / np.max(image)
    tensor = tensor[np.newaxis, :, :, :]
    tensor = torch.from_numpy(tensor).to(device)
    ...
    color_avg = np.random.randint(255, size=(args.max_label_num, 4))
    show = image
    ...
    time1 = time.time() - start_time1
    print('PyTorchInit: %.2f\nTimeUsed: %.2f' % (time0, time1))
    cv2.imwrite(respath+'_c.png', show[:,:,3])
    lab_inverse = lab_inverse.reshape((image.shape[0], image.shape[1])).astype('uint8')
    cv2.imwrite(respath+'.png', lab_inverse)
if __name__ == '__main__':

    # for test
    df = pd.read_csv(r'.\datasets_gf\list_jianshe_pos.txt', header=None, sep=',')
    filelist = df[0].tolist()
    os.makedirs(r'.\datasets_gf\seg_Mynet', exist_ok=True)
    num = len(filelist)
    for i in range(num):
        run(filelist[i], 'seg_Mynet')
