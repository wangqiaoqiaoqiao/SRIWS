...
from TTT_loader import myImageFloder_IRN_pseudo, Segmentation_update,myImageFloder_IRN_update
...
def main():
    ...
    # Setup datalist
    trainlist_pos = r"./datasets_ge/train_list_0.6_jianshe_pos.txt"
    segroot_warm = r'./runs/jianshe_irnet_densent169_ge'
    ...
    testlablist = r'./datasets_ge/test_list_0.6_jianshe.txt'
    SuperPix_dir = r'./datasets_ge/seg_Mynet'
    # Setup parameters
    imgsize = 512
    batch_size = 6
    epochs = 25
    ...
    nchannels = 3
    device = 'cuda'
    logdir = r'.\runs\regnet040_irnet\pos_pseudov2_Classify_densenet169_ge_1'
    global best_acc
    best_acc = 0
    warm_up = 6
    update = 10
    ...
    traindataloader_warm = torch.utils.data.DataLoader(
        myImageFloder_IRN_pseudo(segroot_warm, trainlist_pos, SuperPix_dir, aug=True, channels=nchannels),
        batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    dataloader_update = torch.utils.data.DataLoader(
        Segmentation_update(data_dir=r'D:\wwr\lvwang\ge\jianshe', superpix_root=SuperPix_dir),
        batch_size=batch_size,shuffle=False,num_workers=4,pin_memory=True
    )
    traindataloader_update = torch.utils.data.DataLoader(
        myImageFloder_IRN_update(segroot_warm,trainlist_pos, SuperPix_dir,segroot_update,aug=True,channels=nchannels),
        batch_size=batch_size,shuffle=True,num_workers=4,pin_memory=True
    )
    traindataloader_final = torch.utils.data.DataLoader(
        myImageFloder_IRN_pseudo(segroot_final, trainlist_pos, SuperPix_dir,aug=True, channels=nchannels),
        batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True
    )
    testdataloader = torch.utils.data.DataLoader(
        myImageFloder_IRN_pseudo(segroot_warm, testlablist, SuperPix_dir,aug=False, channels=nchannels),
        batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)
