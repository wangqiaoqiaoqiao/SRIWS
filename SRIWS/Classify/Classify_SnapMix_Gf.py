...
def main():
    ...
    trainlist_neg = os.path.join(iroot, 'train_list_0.6_jianshe_neg.txt')
    testlist = os.path.join(iroot, 'test_list_0.6_jianshe.txt')
    ...
    epoch_warm = 2
    classes = 2 #
    nchannels = 4 # channels
    device = 'cuda'
    logdir = r'.\runs\Classify_CAM_Atten_MIX_1_densenet169_gf'
    ...
    traindataloader_pos = torch.utils.data.DataLoader(
        myImageFloder_Gf(trainlist_pos, aug=True, channels=nchannels),
        batch_size=batch_size//2, shuffle=True, num_workers=4, pin_memory=True)
    traindataloader_neg = torch.utils.data.DataLoader(
        myImageFloder_Gf(trainlist_neg, aug=True, channels=nchannels),
        batch_size=batch_size//2, shuffle=True, num_workers=4, pin_memory=True)

    traindataloader_pos_sample = torch.utils.data.DataLoader(
        myImageFloder_Gf(trainlist_pos, aug=False, channels=nchannels),
    ...
    traindataloader_neg_sample = torch.utils.data.DataLoader(
        myImageFloder_Gf(trainlist_neg, aug=False, channels=nchannels),
        batch_size=batch_size // 2, shuffle=False, num_workers=4, pin_memory=True)

    testdataloader = torch.utils.data.DataLoader(
        myImageFloder_Gf(testlist, aug=False, channels=nchannels),
            ...
            if epoch<epoch_warm:
                ...
                if epoch< epoch_warm+1:
                    ...
                    traindataloader_mix_pos = torch.utils.data.DataLoader(
                        myImageFloder_Gf(trainlist_mix_pos, aug=True, channels=nchannels),
                        batch_size=batch_size // 2, shuffle=True, num_workers=4, pin_memory=True)
    ...
    with torch.no_grad():
        ...
        for x1, y1 in tqdm(dataloader_pos):
            ...
            for i in range(image_new.size(0)):
                image = image_new[i].permute(1, 2, 0).detach().cpu().numpy() * IMG_STD_ALL_Gf + IMG_MEAN_ALL_Gf
                tifffile.imwrite(sample_path + '//' + str(idx) + '_' + str(i) + '_1.tif', image.astype('uint16'))
            ...
            #                     beta=1.0,cropsize=512, netname='regnet', model=model)
            # for i in range(image_new.size(0)):
            #     image = image_new[i].permute(1, 2, 0).detach().cpu().numpy() * IMG_STD_ALL_Gf + IMG_MEAN_ALL_Gf
            #     tifffile.imwrite(sample_path + '//' + str(idx) + '_' + str(i) + '_2.tif', image.astype('uint16'))
            # # # # # #
            ...
            # for i in range(image_new.size(0)):
            #     image = image_new[i].permute(1, 2, 0).detach().cpu().numpy() * IMG_STD_ALL_Gf + IMG_MEAN_ALL_Gf
            #     tifffile.imwrite(sample_path + '//' + str(idx) + '_' + str(i) + '_3.tif', image.astype('uint16'))
            # # # #
            ...
            # for i in range(image_new.size(0)):
            #     image = image_new[i].permute(1, 2, 0).detach().cpu().numpy() * IMG_STD_ALL_Gf + IMG_MEAN_ALL_Gf
            #     tifffile.imwrite(sample_path + '//' + str(idx) + '_' + str(i) + '_4.tif', image.astype('uint16'))
            idx = idx + 1
...
def vtest_epoch(model, criterion, dataloader, device, epoch, classes):
    ...
    with torch.no_grad():
        for idx, (x, y_true) in enumerate(dataloader):
            ...
            pbar.update()
        pbar.close()
    # print('\nx1:',(noise_sum_x1_false / batch_num_x1_false) - (noise_sum_x1_correct / batch_num_x1_correct))
    # print('\nx0:',(noise_sum_x0_false / batch_num_x0_false) - (noise_sum_x0_correct / batch_num_x0_correct))
    oa = acc_total.OverallAccuracy()
    f1 = acc_total.F1score()
